...
interface Creep extends RoomObject
{
	 ...
	 * Can be undefined for enemy creeps.
	 */
	readonly memory: CreepMemory | undefined;
	/**
	 ...
	 */
	attack(target: EnemyCreep | PowerCreep | Structure): CreepActionReturnCode;
	/**
	 ...
	 * @param methodName The name of a creep's method to be canceled.
	 ...
	 */
	heal(target: MyCreep | PowerCreep): CreepActionReturnCode;
	/**
	 ...
	 */
	moveTo(x: number, y: number, opts?: MoveToOptions): CreepMoveReturnCode | ERR_NO_PATH | ERR_INVALID_TARGET;
	/**
	 * Find the optimal path to the target within the same room and move to it.
	 * A shorthand to consequent calls of pos.findPathTo() and move() methods.
	 * If the target is in another room, then the corresponding exit will be used as a target.
	 ...
	 */
	pull(target: MyCreep): OK | ERR_NOT_OWNER | ERR_BUSY | ERR_INVALID_TARGET | ERR_NOT_IN_RANGE | ERR_NO_BODYPART;
	/**
	 ...
	 */
	rangedAttack(target: EnemyCreep | PowerCreep | Structure): CreepActionReturnCode;
	/**
	 ...
	 * @param target The target creep object.
	 */
	rangedHeal(target: MyCreep | PowerCreep): CreepActionReturnCode;
	/**
...
interface Game
{
	 ...
	 */
	creeps: { [creepName: string]: AnyMyCreep; };
	/**
...
declare const Store: _Constructor<Store<ResourceConstant, true>>;
...
interface FindTypes
{
	...
	102: MyCreep; // FIND_MY_CREEPS
	103: EnemyCreep; // FIND_HOSTILE_CREEPS
...
interface MoveToOptions extends FindPathOptions
{
	/**
 ...
 * Your script is executed separately on each shard, and their `Memory` objects are isolated from each other.
...
{
	/**
	 * The prototype is stored in the Mineral.prototype global object. You can use it to extend game objects behavior globally.
	 */
...
interface RoomPosition
{
	...
	findClosestByPath<K extends FindConstant, S extends FindTypes[K]>(
		type: K,
		opts?: FindPathOptions & Partial<FilterOptions<K, S>> & { algorithm?: FindClosestByPathAlgorithm; },
	): S | null;
	findClosestByPath<S extends AnyStructure>(
		type: FIND_STRUCTURES | FIND_MY_STRUCTURES | FIND_HOSTILE_STRUCTURES,
		opts?: FindPathOptions & Partial<FilterOptions<FIND_STRUCTURES, S>> & { algorithm?: FindClosestByPathAlgorithm; },
	): S | null;
	/**
	 * Find the object with the shortest path from the given position. Uses A* search algorithm and Dijkstra's algorithm.
	...
	findClosestByPath<T extends _HasRoomPosition | RoomPosition>(
		objects: T[],
		opts?: FindPathOptions & { filter?: ((object: T) => boolean) | FilterObject | string; algorithm?: FindClosestByPathAlgorithm; },
	): T | null;
	/**
	 ...
	 */
	findPathTo(x: number, y: number, opts?: FindPathOptions): PathStep[];
	/**
	 * Find an optimal path to the specified position using A* search algorithm.
	 *
	 * This method is a shorthand for Room.findPath. If the target is in another room, then the corresponding exit will be used as a target.
	 ...
	 * @param opts An object containing pathfinding options flags (see Room.findPath for more details).
	 */
	findPathTo(target: RoomPosition | _HasRoomPosition, opts?: FindPathOptions): PathStep[];
	/**
...
interface Room
{
	 ...
	 */
	findPath(fromPos: RoomPosition, toPos: RoomPosition, opts?: FindPathOptions): PathStep[];
	/**
	 ...
	 * @returns An object with the structure object[X coord][y coord] as an array of found objects.
	 */
...
{
	/**
	 * The prototype is stored in the Source.prototype global object. You can use it to extend game objects behavior globally:
	 */
...
interface StructureSpawn extends OwnedStructure<STRUCTURE_SPAWN>
{
	 ...
	 * ```
	 */
	spawnCreep(body: readonly BodyPartConstant[], name: string, opts?: SpawnOptions): ScreepsReturnCode;
	 ...
	 * @param target The target creep object.
	 */
	renewCreep(target: MyCreep): ScreepsReturnCode;
	/**
...
interface Spawning
{
	 ...
	 */
	setDirections(directions: readonly DirectionConstant[]): ScreepsReturnCode & (OK | ERR_NOT_OWNER | ERR_INVALID_ARGS);
}
...
interface SpawnOptions
{
	 ...
	 */
	energyStructures?: readonly (StructureSpawn | StructureExtension)[];
	/**
	 ...
	 */
	directions?: readonly DirectionConstant[];
}
...
interface StructureTower extends OwnedStructure<STRUCTURE_TOWER>
{
	 ...
	 */
	attack(target: EnemyCreep | PowerCreep | Structure): ScreepsReturnCode;
	/**
	 ...
	 * @param target The target creep.
	 */
	heal(target: MyCreep | PowerCreep): ScreepsReturnCode;
	/**
...
interface StructureLab extends OwnedStructure<STRUCTURE_LAB>
{
	 ...
	 */
	boostCreep(creep: MyCreep, bodyPartsCount?: number): ScreepsReturnCode;
	/**
 ...
 * until it is landed. Incoming nuke cannot be moved or canceled. Nukes cannot
